...
import {CancelIcon, Cross, GreyCloseIcon} from '../../../assets';
...
export class ToastRender extends React.Component<IProps, IState> {
    ...
    if (this.props.timeout) {
      this._countdown = (() => {
        const _timeout = setTimeout(
          () =>
            this._animatedTiming(-this._heightContainer).start(
              this.props.onClose,
            ),
          this.props.timeout,
        );
  ...
  private _ModelRender = () => {
    const {type, title, description, titleStyle, textBodyStyle, onClose} =
      this.props;
    ...
    return (
      <View style={styles.cardContainer}>
        ...
        <TouchableOpacity
          onPress={onClose}
          // onPress={(this.props.onClose, this._autoCloseHandler)}
        >
          <Cross
            fill="black"
            stroke="black"
      ...
      <Animated.View
        ...
        style={StyleSheet.flatten([
          styles.container,
          {
            ...
            transform: [{translateY: this._positionToast}],
          },
        ])}>
        <_ModelRender />
      </Animated.View>
